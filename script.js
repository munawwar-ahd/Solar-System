...
planets.forEach(planet=>{
  ...
  if(def.small && def.small.count>0){
    ...
    for(let i=0;i<count;i++){
      ...
      // Give each small moon a generated name and metadata for clicking
      const smallMoonName = `S/${pName.slice(0,3).toUpperCase()} ${String(i+1).padStart(2,'0')}`;
      mesh.userData.moonName = smallMoonName;
      mesh.userData.parentPlanet = pName;
...
// Track mouse drag vs click
let mouseDownPos={x:0,y:0};
window.addEventListener('mousedown',e=>{mouseDownPos={x:e.clientX,y:e.clientY};});

window.addEventListener('mousemove',e=>{
  mouse2d.x=(e.clientX/window.innerWidth)*2-1;
  mouse2d.y=-(e.clientY/window.innerHeight)*2+1;
  raycaster.setFromCamera(mouse2d,camera);
  const hits=raycaster.intersectObjects([...planetMeshes,sunMesh,...(showMoons?allMoonMeshes:[])],true);
...
function focusPlanet(planet){
  ...
  document.getElementById('panel-stats').innerHTML=Object.entries(def.stats)
    .filter(([k])=>k!=='type')
    .map(([k,v])=>`<div class="stat-item"><div class="stat-label">${k.toUpperCase()}</div><div class="stat-value">${v}</div></div>`)
    .join('');
  document.getElementById('panel').classList.remove('hidden');
  const pos=planet.mesh.getWorldPosition(new THREE.Vector3());
  const dist=Math.max(def.radius*5.5,10);
  tweenCamera(pos.clone().add(new THREE.Vector3(dist*0.8,dist*0.4,dist*0.8)),pos);
}

// ── Focus: Moon ───────────────────────────────────────
const MOON_TYPES={
  moon_io:'Volcanic Moon', moon_europa:'Icy Moon', moon_ganymede:'Rocky Moon',
  moon_callisto:'Cratered Moon', moon_titan:'Haze Moon', moon_enceladus:'Icy Moon',
  moon_triton:'Retrograde Moon', moon_phobos:'Captured Asteroid', moon_grey:'Irregular Moon',
  moon_icy:'Icy Moon', moon:'Natural Satellite',
};
const MOON_DESC={
  Moon:'Earth\'s only natural satellite',
  Io:'Most volcanically active body in the Solar System',
  Europa:'Subsurface ocean beneath its icy crust',
  Ganymede:'Largest moon in the Solar System',
  Callisto:'Oldest, most cratered surface in the Solar System',
  Titan:'Dense nitrogen atmosphere, methane lakes',
  Enceladus:'Active water-ice geysers at its south pole',
  Triton:'Retrograde orbit — likely a captured Kuiper Belt object',
  Phobos:'Slowly spiraling inward, will crash or break apart',
  Deimos:'Outermost of Mars\' two tiny moons',
  Phoebe:'Retrograde, captured outer moon',
};

function focusMoon(obj){
  const name=obj.userData.moonName;
  const parent=obj.userData.parentPlanet;
  const isSmall=obj.userData.isSmallMoon;
  const texKey=obj.material.map?Object.entries(texCache).find(([k,v])=>v===obj.material.map)?.[0]:'moon_grey';
  const moonType=isSmall?'Irregular Moon':(MOON_TYPES[texKey]||'Natural Satellite');
  const desc=MOON_DESC[name]||`Small irregular moon of ${parent}`;

  // If it's a named moon find its def for orbit data
  const pSys=moonSystems[parent];
  const namedEntry=pSys?.named.find(m=>m.def.name===name);
  const dist=namedEntry?namedEntry.dist.toFixed(1):(obj.userData.moonDist||'—');
  const period=namedEntry?namedEntry.def.period:(null);

  document.getElementById('panel-type').textContent=moonType.toUpperCase();
  document.getElementById('panel-name').textContent=name;
  document.getElementById('panel-stats').innerHTML=`
    <div class="stat-item"><div class="stat-label">PARENT PLANET</div><div class="stat-value">${parent}</div></div>
    <div class="stat-item"><div class="stat-label">ORBIT DIST</div><div class="stat-value">${dist} units</div></div>
    ${period?`<div class="stat-item"><div class="stat-label">PERIOD</div><div class="stat-value">${period<0.1?(period*365.25).toFixed(2)+' days':(period*365.25).toFixed(0)+' days'}</div></div>`:''}
    <div class="stat-item" style="grid-column:1/-1"><div class="stat-label">NOTES</div><div class="stat-value">${desc}</div></div>
  `;
  document.getElementById('panel').classList.remove('hidden');

  // Focus camera on the moon
  const pos=obj.getWorldPosition(new THREE.Vector3());
  const r=obj.geometry.parameters?.radius||0.3;
  const camDist=Math.max(r*12, 3.5);
  tweenCamera(
    pos.clone().add(new THREE.Vector3(camDist*0.8,camDist*0.4,camDist*0.8)),
    pos
  );

  // Track the moon as focused target
  selectedPlanet=null;
  isFocused=true;
  // Store focused moon mesh for follow
  _focusedMoonMesh=obj;
}

let _focusedMoonMesh=null;
...
document.getElementById('panel-close').addEventListener('click',()=>{document.getElementById('panel').classList.add('hidden');selectedPlanet=null;_focusedMoonMesh=null;isFocused=false;});
...
document.getElementById('orbit-toggle').addEventListener('change',e=>{showOrbits=e.target.checked;orbitLines.forEach(l=>l.visible=showOrbits);moonOrbitLines.forEach(l=>l.visible=showOrbits&&showMoons);});
...
document.getElementById('moons-toggle').addEventListener('change',e=>{
  showMoons=e.target.checked;
  // Toggle all named moon meshes
  Object.values(moonSystems).forEach(sys=>{
    sys.named.forEach(m=>{
      m.mesh.visible=showMoons;
      m.orbitPath.visible=showMoons&&showOrbits;
    });
    sys.small.forEach(m=>{ m.mesh.visible=showMoons; });
  });
  // If moons hidden and a moon is focused, close panel and release focus
  if(!showMoons&&_focusedMoonMesh){
    document.getElementById('panel').classList.add('hidden');
    _focusedMoonMesh=null; isFocused=false;
  }
});
...
function animate(){
  ...
  else if(isFocused&&_focusedMoonMesh){_focusedMoonMesh.getWorldPosition(_wp);controls.target.lerp(_wp,0.06);}
  controls.update();
